...
import { putSkill } from "../../../services/services";

const EditSkillModal = ({ handleChange, formData }) => {
  const handlePutSkill = async () => {
    try {
      await putSkill(formData.Skill_ID, formData.Skill, formData.Ca_ID);
    } catch (error) {
      // Manejo de errores
    }
  };
